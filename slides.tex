...
The \texttt{.git} directory is the repository, everything else is the \emph{working directory}.
  \begin{center}
...
\begin{frame}{Commits}
  \begin{itemize}
  \item Git stores whole files in the repository, not differences. Thus a \emph{commit} is a snapshot of tracked files at given time.
  \item Each commit is identified by its hash (SHA-1). There are no sequential revision numbers.
  \item Commits contain pointers to their parent commit(s).
  \end{itemize}
\end{frame}

%TODO: replace picture with v2 from Git book, it's much better
...
\begin{frame}{Merging}
  ...
  \item \emph{Rebasing} helps keeping the history linear (explained later).
  \end{itemize}
\end{frame}

\begin{frame}{Merging: fast-forward}
  \begin{center}
    \only<+>{\includegraphics{images/branch3.png}}
    \only<+>{\includegraphics{images/branch4.png}}
  \end{center}
  To merge \texttt{hotfix} into \texttt{master}, Git simply moves the branch pointer forward.\\There cannot be conflicts in this case.
\end{frame}

\begin{frame}{Merging: three-way}
  \begin{center}
    \only<+>{\includegraphics{images/branch5.png}}
    \only<+>{\includegraphics{images/branch6.png}}
  \end{center}
  To merge \texttt{iss53} into \texttt{master}, Git finds the best common ancestor of the two branches then creates an additional \emph{merge commit} to hold the result.\\Conflicts happen when the same part of the same file is changed differently in both branches.
\end{frame}
...
\begin{frame}{Rebasing: a cleaner way to integrate changes}
  \only<+>{
    \begin{columns}[c]
      \begin{column}{5cm}
        \begin{center}
          \includegraphics[scale=0.8]{images/rebase0.png}\\1) Initial situation: two divergent branches
        \end{center}
      \end{column}
      \begin{column}{6cm}
        \begin{center}
          \includegraphics[scale=0.8]{images/rebase1.png}\\2a) \emph{Merge} experiment into master
          \vskip 0.8em
          \hrule
          \vskip 0.8em
          \includegraphics[scale=0.8]{images/rebase2.png}\\2b) \emph{Rebase} experiment onto master
        \end{center}
      \end{column}
    \end{columns}
  }
  \only<+>{
    \begin{center}
      \includegraphics{images/rebase2.png}
    \end{center}
    \begin{itemize}
...
\begin{frame}{History}
  \begin{block}{Viewing the commit history}
    ...
    \item Has many options to customise output:
      ...
      \item[decorate] Adds branch and tag names on the commit they point at
...
\begin{frame}{Branching and merging}
  \only<+>{
    \begin{block}{Merging two branches}
      ...
      \begin{itemize}
      \item Merges changes from given branch into current branch
